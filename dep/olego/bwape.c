...
static int pairing(bwa_seq_t *p[2], pe_data_t *d, const pe_opt_t *opt, int s_mm, const isize_info_t *ii, const int *g_log_n)
{
...
#define __pairing_aux2(q, w) do {										\
		...
		if ((q)->pos != (w)>>32 || (q)->strand != r->a) {				\
			(q)->n_mm = r->n_mm; \
			(q)->n_gapo_t = r->n_gapo_t; (q)->n_gape_t = r->n_gape_t;	\
			(q)->n_gapo_q = r->n_gapo_q; (q)->n_gape_q = r->n_gape_q;	\
			(q)->strand = r->a; \
			(q)->score = r->score;										\
			(q)->pos = (w)>>32;											\
			if ((q)->mapQ > 0) ++cnt_chg;								\
...
int bwa_cal_pac_pos_pe(const char *prefix, bwt_t *const _bwt[2], int n_seqs, bwa_seq_t *seqs[2], FILE *fp_sa[2], isize_info_t *ii,
					   const pe_opt_t *opt, const gap_opt_t *gopt, const isize_info_t *last_ii, const int *g_log_n)
{
	int i, j, cnt_chg = 0;
	char str[1024];
	bwt_t *bwt[2];
	...
	for (i = 0; i != n_seqs; ++i) {
		...
		for (j = 0; j < 2; ++j) {
			...
			if (p[j]->type == BWA_TYPE_UNIQUE || p[j]->type == BWA_TYPE_REPEAT) {
				...
				p[j]->seQ = p[j]->mapQ = bwa_approx_mapQ(p[j], max_diff, g_log_n);
			}
		}
	}
	...
	for (i = 0; i != n_seqs; ++i) {
		...
		{ // only when both ends mapped
			...
			cnt_chg += pairing(p, d, opt, gopt->s_mm, ii, g_log_n);
		}
					  ...
					  int *n_cigar, uint64_t *_cnt)
{
	...
	{ // set *cnt
		int n_mm, n_gapo_t, n_gape_t, n_gapo_q, n_gape_q;
		n_mm = n_gapo_t = n_gape_t = n_gapo_q = n_gape_q = 0;
		p = path + path_len - 1;
		...
		for (k = 0; k < *n_cigar; ++k) {
			...
			} else if (__cigar_op(c) == FROM_D) { //TO DO: deletion in reference? to be tested !!!
				x += __cigar_len(c), ++n_gapo_t, n_gape_t += (__cigar_len(c)) - 1;
			} else if (__cigar_op(c) == FROM_I) { //deletion in query
				y += __cigar_len(c), ++n_gapo_q, n_gape_q += (__cigar_len(c)) - 1;
			}
		}
		*_cnt = (uint64_t)n_mm << 32 | n_gapo_t<<24 | n_gape_t << 16 | n_gapo_q << 8 | n_gape_q;
	}
...
#define __set_fixed(_pref, _pmate, _beg, _cnt) do {						\
				...
				_pmate->n_mm = _cnt>>32; 								\
				_pmate->n_gapo_t = _cnt>>24&0xff; _pmate->n_gape_t = _cnt>>16&0xff; \
				_pmate->n_gapo_q = _cnt>>8&0xff; _pmate->n_gape_q = _cnt&0xff; \
			...
			for (k = 0; k < 2; ++k) { // p[1-k] is the reference read and p[k] is the read considered to be modified
				...
				if (cigar[k] && p[k]->type != BWA_TYPE_NO_MATCH) { // re-evaluate cigar[k]
					...
					s_old = (int)((p[k]->n_mm * 9 + (p[k]->n_gapo_t+p[k]->n_gapo_q) * 13 + (p[k]->n_gape_t+p[k]->n_gape_q) * 2) / 3. * 8. + .499);

					s_new = (int)(((cnt[k]>>32) * 9 // mismatch
							+ ((cnt[k]>>24&0xff) + (cnt[k]>>8&0xff)) * 13 //gap opening
							+ ((cnt[k]>>16&0xff) + (cnt[k]&0xff)) * 2 //gap extension
							+ clip * 3) / 3. * 8. + .499);
				...
				// now reverse sequence back such that p[*]->seq looks untouched
				if (popt->type == BWA_PET_STD) {
...
void bwa_sai2sam_pe_core(const char *prefix, char *const fn_sa[2], char *const fn_fa[2], pe_opt_t *popt, const char *bwa_rg)
{
	extern bwa_seqio_t *bwa_open_reads(int mode, const char *fn_fa);
	extern int bwa_set_rg(const char *s, char **bwa_rg_id, char **bwa_rg_line);
	...
	int g_log_n[256];
	char *bwa_rg_id = 0, *bwa_rg_line = 0;


	if (bwa_rg && bwa_set_rg(bwa_rg, &bwa_rg_id, &bwa_rg_line) < 0) {
		fprintf(stderr, "[%s] malformated @RG line\n", __func__);
		exit(1);
	}
	
	// initialization
	bwase_initialize(g_log_n); // initialize g_log_n[] in bwase.c
	pac = 0; bwt[0] = bwt[1] = 0;
	...
	while ((seqs[0] = bwa_read_seq(ks[0], 0x40000, &n_seqs, opt.mode & BWA_MODE_COMPREAD, opt.trim_qual)) != 0) {
		...
		cnt_chg = bwa_cal_pac_pos_pe(prefix, bwt, n_seqs, seqs, fp_sa, &ii, popt, &opt, &last_ii, g_log_n);
...
int bwa_sai2sam_pe(int argc, char *argv[])
{
	...
	bwa_sai2sam_pe_core(argv[optind], argv + optind + 1, argv + optind+3, popt, bwa_rg);
	free(popt);
	return 0;
}
