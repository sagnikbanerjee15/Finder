...
static inline void gap_push(gap_stack_t *stack, int a, int i, bwtint_t k, bwtint_t l,
		int n_mm, int n_gapo_t, int n_gape_t, int n_gapo_q, int n_gape_q,
							int state, int is_diff, const gap_opt_t *opt)
{
	...
	//sum up the gaps in target and query
	int n_gapo = n_gapo_t + n_gapo_q;
	int n_gape = n_gape_t + n_gape_q;
	...
	p->n_mm = n_mm; p->n_gapo_t = n_gapo_t; p->n_gape_t = n_gape_t; p->n_gapo_q = n_gapo_q; p->n_gape_q = n_gape_q;
	p->state = state;
...
static inline void gap_shadow(uint32_t x, int len, bwtint_t max, int last_diff_pos, bwt_width_t *w)
{
	int i, j;
...
bwt_aln1_t *bwt_match_gap(const bwt_t *bwt, uint32_t len, const ubyte_t *seq[2], bwt_width_t *w[2],
						  /*bwt_width_t *seed_w[2],*/ const gap_opt_t *opt, int *_n_aln, gap_stack_t *stack)
{
	uint32_t best_score = aln_score(opt->max_diff+1, opt->max_gapo+1, opt->max_gape+1, opt);
	uint32_t best_diff = opt->max_diff + 1, max_diff = opt->max_diff;
	uint32_t j, _j, n_aln, m_aln;
	int best_cnt = 0, max_entries = 0;
	...
	gap_push(stack, 0, len, 0, bwt->seq_len, 0, 0, 0, 0, 0, 0, 0, opt);
	gap_push(stack, 1, len, 0, bwt->seq_len, 0, 0, 0, 0, 0, 0, 0, opt);
	...
	while (stack->n_entries) {
		...
		m = max_diff - (e.n_mm + e.n_gapo_t + e.n_gapo_q);
		if (opt->mode & BWA_MODE_GAPE) m -= (e.n_gape_t + e.n_gape_q);
		if (m < 0) continue;
		//bwt = bwts[1-a];
		//bwt = bwts[0]; // always use the forward bwt now
		str = seq[a]; width = w[a];
	//	if (seed_w) { // apply seeding
	//		seed_width = seed_w[a];
	//		m_seed = opt->max_seed_diff - (e.n_mm + e.n_gapo_t + e.n_gapo_q);
	//		if (opt->mode & BWA_MODE_GAPE) m_seed -= (e.n_gape_t + e.n_gape_q);
	//	}
		...
		if (i == 0) hit_found = 1;
				...
				&& (e.state == STATE_M || (opt->mode&BWA_MODE_GAPE)
					|| (e.n_gape_t + e.n_gape_q) == opt->max_gape)) { // no diff allowed
			if (bwt_match_exact_alt(bwt, i, str, &k, &l)) hit_found = 1;
			else continue; // no hit, skip
		}

		if (hit_found) { // action for found hits
			uint32_t score = aln_score(e.n_mm, e.n_gapo_t + e.n_gapo_q, e.n_gape_t + e.n_gape_q, opt);
			int do_add = 1;
			//printf("#2 hits found: %d:(%u,%u)\n", e.n_mm+e.n_gapo, k, l);
			if (n_aln == 0) {
				best_score = score;
				best_diff = e.n_mm + e.n_gapo_t + e.n_gapo_q;
				if (opt->mode & BWA_MODE_GAPE) best_diff += e.n_gape_t + e.n_gape_q;
			...
			if (e.n_gapo_t + e.n_gapo_q) { // check whether the hit has been found. this may happen when a gap occurs in a tandem repeat
				for (j = 0; j != n_aln; ++j)
			...
			if (do_add) { // append
				...
				p = aln + n_aln;
				p->n_mm = e.n_mm;
				p->n_gapo_t = e.n_gapo_t; p->n_gape_t = e.n_gape_t;
				p->n_gapo_q = e.n_gapo_q; p->n_gape_q = e.n_gape_q;
				p->a = a;
				p->k = k; p->l = l;
				p->score = score;
				++n_aln;
		...
		// indels
		int n_gap = e.n_gapo_t + e.n_gape_t + e.n_gapo_q + e.n_gape_q;
		tmp = (opt->mode & BWA_MODE_LOGGAP)? int_log2(n_gap)/2+1 : n_gap;
		if (allow_diff && i >= opt->indel_end_skip + tmp && (int)len - i >= opt->indel_end_skip + tmp) {
			if (e.state == STATE_M) { // gap open
				if (e.n_gapo_t + e.n_gapo_q < opt->max_gapo) { // gap open is allowed
					// insertion in genome, or in the other word, gap in query n_gapo_q++

					gap_push(stack, a, i, k, l, e.n_mm, e.n_gapo_t+1, e.n_gape_t, e.n_gapo_q, e.n_gape_q, STATE_I, 1, opt);
					// deletion in the genome, or in other words insertion in query, n_gapo_t++
					for (j = 0; j != 4; ++j) {
						k = bwt->L2[j] + cnt_k[j] + 1;
						l = bwt->L2[j] + cnt_l[j];
						if (k <= l) gap_push(stack, a, i + 1, k, l,
								e.n_mm, e.n_gapo_t, e.n_gape_t, e.n_gapo_q+1, e.n_gape_q, STATE_D, 1, opt);
					}
				}
			} else if (e.state == STATE_I) { // extention of an insertion
				if (e.n_gape_t + e.n_gape_q < opt->max_gape) // gap extension is allowed, n_gape_q++
					gap_push(stack, a, i, k, l, e.n_mm, e.n_gapo_t, e.n_gape_t + 1, e.n_gapo_q, e.n_gape_q, STATE_I, 1, opt);
			} else if (e.state == STATE_D) { // extention of a deletion
				if (e.n_gape_t + e.n_gape_q < opt->max_gape) { // gap extension is allowed n_gape_t++
					if ((uint32_t)(e.n_gape_t + e.n_gapo_t + e.n_gape_q + e.n_gapo_q) < max_diff || (int)occ < opt->max_del_occ) {
						for (j = 0; j != 4; ++j) {
							k = bwt->L2[j] + cnt_k[j] + 1;
							l = bwt->L2[j] + cnt_l[j];
							if (k <= l) gap_push(stack, a, i + 1, k, l,
									e.n_mm, e.n_gapo_t, e.n_gape_t, e.n_gapo_q, e.n_gape_q + 1, STATE_D, 1, opt);
						}
					}
				}
			}
		...
		if (allow_diff && allow_M) { // mismatch is allowed
			for (j = 1; j <= 4; ++j) {
				...
				l = bwt->L2[c] + cnt_l[c];
				if (k <= l) gap_push(stack, a, i, k, l, e.n_mm + is_mm, e.n_gapo_t, e.n_gape_t, e.n_gapo_q, e.n_gape_q, STATE_M, is_mm, opt);
			}
		} else if (str[i] < 4) { // try exact match only
			int c = str[i] & 3;
			k = bwt->L2[c] + cnt_k[c] + 1;
			l = bwt->L2[c] + cnt_l[c];
			if (k <= l) gap_push(stack, a, i, k, l, e.n_mm, e.n_gapo_t, e.n_gape_t, e.n_gapo_q, e.n_gape_q, STATE_M, 0, opt);
		}
	}
